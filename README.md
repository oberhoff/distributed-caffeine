...
[near real-time](https://www.mongodb.com/docs/manual/changeStreams/#change-streams) access to data changes. Therefore,
related cache instances share a MongoDB collection to enable distributed synchronization and persistence of cache
entries. Several (customizable) serialization formats are available for storing different contents (for keys and values)
of cache entries in the MongoDB collection.
...
Serialization can be customized (independently for keys and values) by implementing one of the `ByteArraySerializer`,
`StringSerializer` or `JsonSerializer` interfaces.

##### Configuration of a distributed loading cache with additional configuration of extended persistence
